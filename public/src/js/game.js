...
function aplicarBackgroundPorCategoria(categoria) {
    const body = document.body;

    switch (categoria.toLowerCase()) {
        case "animais":
            body.style.backgroundImage = 'url("/public/assets/images/animaisback.png")';
            break;

        case "alimento":
            body.style.backgroundImage = 'url("/public/assets/images/alimentosback.png")';
            break;

        case "profissao":
            body.style.backgroundImage = 'url("/public/assets/images/profissoesback.png")';
            break;

        case "esportes":
            body.style.backgroundImage = 'url("/public/assets/images/esportesback.png")';
            break;


        case "paises":
            body.style.backgroundImage = 'url("/public/assets/images/paisesback.png")';
            break;

        default:
            body.style.backgroundImage = 'url("/public/assets/images/background.png")';
    }

    // estilos adicionais para manter o layout bonito
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    aplicarBackgroundPorCategoria(categoria);
    ...
    switch (resultado.tipo) {
        case 'vidaExtra':
            ...
            if (resultado.jogador === meuNumeroJogador) {
                mostrarFeedback('Prazo extendido! você ganhou uma vida extra', 'green');
        ...
        case 'tirarVida':
            ...
            if (resultado.jogador === meuNumeroJogador) {
                mostrarFeedback(`Aponte o dedo e tente enforcar seu aliado!`, 'green');
                if (resultado.adversarioPerdeuVida) {
                    mostrarFeedback('💥 Adversário foi de arrasta', 'green');
                }
...
//JAVASCRIPT DA ANIMAÇÃO DO PODER
function mostrarCartaDoPoder(poderId) {
  const carta = document.getElementById("carta");
  const video = document.getElementById("videoCarta");
  const poderInfo = MAPEAMENTO_PODERES[poderId];

  if (!carta || !video) {
    console.warn("Elementos da carta/video não encontrados no DOM.");
    return;
  }
  if (!poderInfo || !poderInfo.video) {
    console.warn(`Poder ${poderId} não tem vídeo configurado`);
    return;
  }

  // remove classe caso esteja (garante estado previsível)
  carta.classList.remove("mostrar");

  // limpa src anterior e listeners antigos
  video.pause();
  video.removeAttribute("src");
  video.load();

  // define novo src
  video.src = poderInfo.video;
  video.currentTime = 0;

  // Handler único para quando o vídeo terminar
  function onEndedHandler() {
    // remove a classe mostrando (carta fecha)
    carta.classList.remove("mostrar");

    // limpa handler para não acumular
    video.removeEventListener("ended", onEndedHandler);

    // opcional: limpar src para liberar memória
    // video.removeAttribute('src'); video.load();
  }

  // garante que não existam múltiplos listeners duplicados
  video.removeEventListener("ended", onEndedHandler);
  video.addEventListener("ended", onEndedHandler);

  // Fallback: se o evento 'ended' não ocorrer por alguma razão,
  // remove a carta após video.duration + 0.5s (apenas se duration for válido)
  let fallbackTimeout = null;
  function setFallback() {
    if (!isFinite(video.duration) || video.duration <= 0) return;
    fallbackTimeout = setTimeout(() => {
      // se ainda estiver aberta, fecha
  ...
  // Quando metadata/carregamento estiver pronto, toca e seta fallback
  function onLoadedMeta() {
    // mostra a carta (ativa transição)
    ...
    video.play().catch(err => {
      ...
      // mesmo se bloqueado, mostra a carta — o usuário pode clicar para tocar
    });
  ...
  // Como fallback caso o loadedmetadata já esteja disponível (caching)
  if (video.readyState >= 1) { // HAVE_METADATA
    onLoadedMeta();
  }

  // limpa timeout se o vídeo terminar de forma normal
...
async function usarPoder(poderId, botaoElemento) {
    ...
    //dispara funçao da animaçao da carta
    mostrarCartaDoPoder(poderId);    
        ...
        if (evento.tipo === 'inicio') {
            ...
            if (evento.atualizarVidas && evento.vidas) {
                ...
                if (evento.jogador === meuNumeroJogador) {
                    console.log('haja duplicação de evento hein');
                } else {
...
function atualizarBonecosUI() {
    ...
    log(`🖼️ Bonecos atualizados: J1 (${errosJogador1} erros) -> play1${indiceP1}.png, J2 (${errosJogador2} erros) -> play2${indiceP2}.png`);
}
